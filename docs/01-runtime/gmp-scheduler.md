...
**Магическое число 61**: проверка GRQ каждые 61 тиков предотвращает starvation глобальных горутин при активном work stealing.
...
4. **GOMAXPROCS** — не серебряная пуля. Для CPU-bound `NumCPU()` оптимален, для IO-bound может быть больше
